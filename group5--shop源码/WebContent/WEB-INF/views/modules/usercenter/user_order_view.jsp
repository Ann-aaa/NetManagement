    ...
    <div class="uc-content-box order-view-box">
      <div class="box-hd">
        ...
        <div class="more clearfix">
          ...
          <div class="actions">
            <c:if test="${orderVO.orderStatus eq 1}"><a id="J_payOrder" class="btn btn-small btn-primary" title="立即支付" href="${ctx}/buy/confirm/${orderVO.orderNumber}" target="_blank" data-order-id="${orderVO.orderNumber}">立即支付</a> </c:if>
            ...
            <div class="order-summary">
              <div class="order-status">
                <c:if test="${orderVO.orderStatus eq 1}">等待付款</c:if>
                  ...
                  <c:if test="${orderVO.orderStatus eq 4 || orderVO.orderStatus eq 5}">配送中</c:if>
                <c:if test="${orderVO.orderStatus eq 6}">已收货</c:if>
              ...
              <div class="order-progress">
                ...
                <!--    <li class="step step-first ${status == true ? 'step-done':''}">
                  ...
                  <li class="step step-first ${status == true ? 'step-done':''}">
                    <div><span class="text">已付款</span></div>
                    <div class="info">
                      <fmt:formatDate value="${info}" pattern="yyyy年MM月dd日 hh:mm" />
                    </div>
                  </li>
                  ...
                  <c:set var="info"/>
                  <c:forEach items="${orderVO.orderStatusList}" var="orderStatus">
                    <c:if test="${orderStatus.orderStatus eq 5}">
                      <c:set var="status" value="true"/>
                      <c:set var="info" value="${orderStatus.createTime}"/>
                    </c:if>
                  </c:forEach>
                  <li class="step step-first ${status == true ? 'step-done':''}">
                    <div><span class="text">运输中</span></div>
                    <div class="info">
                      <fmt:formatDate value="${info}" pattern="yyyy年MM月dd日 hh:mm" />
                    </div>
                  </li>
          ...
          <div id="editAddr" class="order-detail-info">
            ...
            <table class="info-table">
              <tbody>
                <tr>
                  <th>姓&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;名：</th>
                  <td>${orderVO.osOrderShipment.userName}</td>
                </tr>
                <tr>
                  <th>联系电话：</th>
                  <td>${orderVO.osOrderShipment.userPhone}</td>
                </tr>
                <tr>
                  <th>收货地址：</th>
                  <td>&nbsp;${orderVO.osOrderShipment.userAdress}</td>
                </tr>
              </tbody>
            </table>
            <!-- <div class="actions">
               <c:if test="${orderVO.orderStatus eq 1}"><a class="btn btn-small btn-line-gray J_editAddr" data-shipment-id="${orderVO.osOrderShipment.orderShipmentId}" data-order-id="${orderVO.orderId}"> 修改 </a></c:if>
            </div>-->
          </div>
...
<div id="J_modalVerify" class="modal modal-verify modal-hide in disabled" aria-hidden="false" style="display: none;" ">
  <div class="modal-hd">
    ...
    <form id="J_updateAddrForm" class="form-update-addr" method="post" action="">
      <div class="form-row  clearfix">
        <fieldset class="form-section form-section-active">
          ...
          <input class="input-text" type="text" id="Consignee" name="Order[consignee]" value="${orderVO.osOrderShipment.userName}" maxlength="15" placeholder="必填" required>
        </fieldset>
        <fieldset class="form-section form-section-active">
          <label class="input-label" for="Telephone">联系电话</label>
          <input class="input-text" type="text" value="${orderVO.osOrderShipment.userPhone}" id="Telephone" name="Order[tel]" placeholder="11位手机号（必填）" required>
      ...
      <div class="form-row clearfix">
        <fieldset class="form-section form-section-active">
          ...
          <textarea class="input-text" id="Street" name="Order[address]" placeholder="路名或街道地址，门牌号">${orderVO.osOrderShipment.userAdress}</textarea>
        </fieldset>
      </div>
      <div class="form-row clearfix">
        <fieldset class="form-section form-section-active">
          <label class="input-label" for="Zipcode">邮政编码</label>
          <input class="input-text" type="text" id="Zipcode" name="Order[zipcode]" value="${orderVO.osOrderShipment.userZipcode}" maxlength="6" placeholder="必填" required>
        </fieldset>
      </div>
