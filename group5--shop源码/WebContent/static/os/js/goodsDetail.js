    ...
    function getattrprice() {
        ...
        if (!!defaultstats) {
            if (typeof (sys_item[_val]) == "undefined") {
                $('#goodsDetailBtnBox').css('display', 'none');//不显示
                $('#goodsDetailBtnBoxForInform').css('display', 'block');//显示
            } else {
        ...
        $("#goodsDetailUpdateSortBtn").attr("data-product-spec-number", _productSpecNumber);
...
$(function () {
    if ($('#J_goodsInfoBlock').children('.sys_item_specpara').length <= 0) {
        ...
        $(_resp.score).text(_score); // 其中的math.round为截取小数点位数
        $(_resp.price).text(_price);
        $("#goodsDetailAddCartBtn").attr("data-product-spec-number", _productSpecNumber);//加入购物车属性设置值为 _productSpecNumber
        $("#goodsDetailUpdateSortBtn").attr("data-product-spec-number", _productSpecNumber);
        $("#goodsSubBarAddCartBtn").attr("data-product-spec-number", _productSpecNumber);
    }
})
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.code == 1) {
            	...
            	window.location.href = baselocation + '/cart/phone/' + result.message;
            }else if (result.code == 3) {
            	window.location.href = baselocation + '/cart/otherpro/' + result.message;
            }
...
function update_sort(obj) {
    ...
    $.ajax({
        ...
        success: function (result) {
            console.log(result.message);
            if (result.code == 1) {
                window.location.href = baselocation + '/cart/updatesort/' + result.message;     
            }
            else if (result.code == 401) {
                window.location.href = baselocation + '/pass/login';
            }
