	...
	public CartVO insertProductCart(Long productSpecNumber, HttpSession session) {
		...
		if (osProductSpecification != null && (!"".equals(osProductSpecification.getSpec()))) {
			...
			for (ShoppingCartVO vo : shoppingCartVOs) {
				if (osProductSpecification.getProductSpecNumber()
						.longValue() == (vo.getProductSpecNumber().longValue())) {
			...
			if (flag == false) {
				...
				shoppingCartVO.setProductNumber(
						osProductMapper.selectProductNumberByProductId(osProductSpecification.getProductId()));
				shoppingCartVO.setName(osProductMapper.selectNameByProductId(osProductSpecification.getProductId()));
				shoppingCartVO
						.setPicImg(osProductMapper.selectPicImgByProductId(osProductSpecification.getProductId()));
				shoppingCartVO
						.setSellerId(osProductMapper.selectSellerIdByProductId(osProductSpecification.getProductId()));
				// 规格名称处理
				...
				if (s != null && (!s.equals(""))) {
					String[] sp = s.split(",");
					for (String ss : sp) {
						OsSpecificationAttribute osSpecificationAttribute = osSpecificationAttributeMapper
								.selectByPrimaryKey(Long.parseLong(ss));
